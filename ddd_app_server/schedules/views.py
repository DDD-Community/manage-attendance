...
from .mixins import CurrentScheduleMixin, CurrentUserMixin
    ...
    def get(self, request, *args, **kwargs):
        if request.user.is_staff:
            ...
            schedules = Schedule.objects.filter(
                ...
                Q(assigned_groups__name__in=request.user.groups.values_list('name', flat=True))  # TODO remove unused filter
            ).distinct()
...
class AttendanceDetailView(BaseResponseMixin, CurrentScheduleMixin, CurrentUserMixin, APIView):
    permission_classes = [permissions.IsAuthenticated]
    authentication_classes = [JWTAuthentication]
    serializer_class = AttendanceSerializer
